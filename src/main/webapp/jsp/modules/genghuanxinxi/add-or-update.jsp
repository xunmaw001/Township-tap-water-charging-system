                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            <div class="form-row">
                                   ...
                                   <div class="form-group col-md-6">
                                       <label>更换水表</label>
                                       <select id="sbTypesSelect" name="sbTypes" class="form-control">
                                       </select>
                                   </div>
                                    <div class="form-group col-md-6">
                                        <label>更换原因</label>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("genghuanxinxi/" + urlParam, "POST", data, (res) => {
                if(res.code == 0
        )
            {
    ...
    function initializationSbtypesSelect(){
        var sbTypesSelect = document.getElementById('sbTypesSelect');
            for (var i = 0; i < sbTypesOptions.length; i++) {
                sbTypesSelect.add(new Option(sbTypesOptions[i].sbname,sbTypesOptions[i].id));
            }
    }
    // 下拉框选项回显
    function setSelectOption() {
        ...
        for (var i = 0; i < sbTypesOptions.length; i++) {
            if(sbTypesOptions[i].id == ruleForm.sbTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("sbTypesSelect").options[i].selected = true;
            }
        }
    }
    ...
    function getDetails() {
        ...
        if (id != null && id != "" && id != "null") {
            updateId = id;
            window.sessionStorage.removeItem('updateId');
            http("genghuanxinxi/info/" + id, "GET", {}, (res) => {
                if(res.code == 0
        )
            {
                ruleForm = res.data
    ...
    function dataBind() {
        ...
        $("#changecause").val(ruleForm.changecause);
        $("#address").val(ruleForm.address);
        $("#contact").val(ruleForm.contact);
